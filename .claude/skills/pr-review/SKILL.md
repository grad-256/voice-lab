...
品質チェックは CLAUDE.md の「品質評価基準（ハーネスパターン）」セクションが
...
plugin のレビュー結果に、CLAUDE.md の「品質評価基準（ハーネスパターン）」
...
- **音声フロー品質**（音声関連のみ）：
  - Whisper → Claude Haiku → ElevenLabs パイプラインの接続
...
| 判定 | 条件 |
|---|---|
| ✅ 問題なし | CLAUDE.md の必須チェックが全通過 + `pnpm test` 通過 + plugin のレビュー指摘なし |
...
raw `gh pr diff` で差分を見つつ CLAUDE.md の品質評価基準セクションで
手動レビューする。その場合もマージ方針（Claude はマージしない）は変わらず。
