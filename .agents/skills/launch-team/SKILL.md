...
description: |
  ...
  **Masaru さんは tmux split で進捗を見たい人なので、何も指定がない場合は必ずパターン A（tmux split）を使う。**
  ...
  使い分ける。Codex の native Agent Teams API（TeamCreate / TaskCreate / SendMessage）と
...
> **前提**: `.Codex/settings.json` に `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` + `teammateMmode: "tmux"` 設定済み。
...
- 「AGENTS.md を他プロジェクトの例と比較してブラッシュアップ案を出して」
...
- `app/` / `components/` / `lib/` 配下の `.ts`/`.tsx` を Edit/Write すると
  `.Codex/flags/evaluator-pending` フラグが立つ
- ターン終了時（Stop フック）にフラグがあれば、Codex に「evaluator を起動せよ」と指示が入る
...
| 自動evaluatorが走らない | `.Codex/settings.json` の hooks 設定確認、`.Codex/flags/` ディレクトリ存在確認 |
| evaluator が無限ループ | `.Codex/flags/evaluator-pending` を手動削除 |
