...
```
/sessions/loving-friendly-cerf/mnt/voice-lab/AGENTS.md
```
...
### Phase 3（最終目標の一つ）
**ゴール：** 自分のサービスを外部 AI（Codex Desktop・Cursor 等）から呼び出せる MCP サーバーとして公開する
...
**完了定義：** Codex Desktop から MCP サーバーに接続し、キャラを作って音声会話できる状態
...
コスト目安：1 分の会話 ≈ $0.01（Whisper + Codex + ElevenLabs 合算）
...
**核心：Phase 0 で作った Whisper → Codex → ElevenLabs の側（パイプライン）は全サービス共通。プロンプトを変えるだけで別サービスになる。**
...
| Codex との効果的な働き方 | https://www.notion.so/32b481455c788193a295f6f17ad05b2f |
...
| Codex API error: credit | Anthropic クレジット不足 | console.anthropic.com で追加 |
