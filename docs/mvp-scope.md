...
9. **サジェスト機能を新規追加する。** 出現タイミング（会話前 / 会話中 / 両方）は**ユーザーが設定画面で選択可**。自動再生は提供しない（ユーザー操作で再生）。
...
| 設定画面（`app/settings/*`） | 維持＋拡張（サジェストタイミング・分身声の再マッチング導線） |
...
#### 出現方式：**プル型**（Sprint 6 改訂）

ユーザーが会話画面で「💡 フレーズのヒント」ボタンを押したときだけサジェストパネルを展開する。自動表示（before_chat / during_chat のプッシュ型）とユーザー設定（`suggest_timing`）は提供しない。

| ユーザーの状況 | `/api/suggest` に渡す `timing` |
|---|---|
| 会話未開始（`messages.length === 0`） | `before_chat`（オープナー候補を生成） |
| 会話中（AI 返答あり） | `during_chat`（次の一言候補を生成） |

録音・処理中はボタンを隠す（`canRequestSuggest` で制御）。パネルは右上 × ボタン / プレフィル完了で自動クローズ。
...
- 入力：`persona_id`、直近の会話履歴（最新 N ターン）、`timing`（`before_chat` / `during_chat`、呼び出し時点の会話状態に応じてクライアント側で決定）。
...
- **`<SuggestPanel />`**：プル型サジェストパネル。親から `active` / `timing` / 直近履歴を受け取り、`/api/suggest` を呼ぶ。`onClose` で × ボタンを有効化。
...
- **TTS**：既存 `/api/speak` を使用。リクエスト body に `voiceId` / `modelId` を渡す仕組みは実装済。既定モデルは `eleven_multilingual_v2`（試聴・会話ラリー）、日記要約と場面学習再生は `eleven_v3` を明示。
...
- **`user_settings`**：Sprint 6 で新設予定だったが、サジェスト自動表示を廃止しプル型に転換したため、現時点では不要（将来別の設定項目が出た時に導入を再検討）。
...
| `suggest_requested` | `timing` (`before_chat`/`during_chat`) | ユーザーが「フレーズのヒント」ボタンでパネルを開いた |
| `suggest_shown` | `timing`, `phrase_count` | サジェスト表示（API 成功後） |
...
- [ ] 呼び出し時の `timing` 引数はクライアント側で会話状態から決定（ユーザー設定は持たない・Sprint 6 で改訂）
...
### Sprint 6：サジェスト UX 改訂（プル型・オンデマンド） `[FE]`

**ゴール**：会話画面の自動サジェスト表示を廃止し、ユーザーがボタンを押した時だけサジェストを出す。
**依存**：Sprint 5
**完了条件**：
- [ ] 会話画面に「💡 フレーズのヒント」ボタンを追加（録音・処理中は隠す）
- [ ] ボタン押下で `<SuggestPanel />` をオンデマンド展開。`timing` は会話状態で自動決定（`messages.length === 0` → `before_chat` / else → `during_chat`）
- [ ] パネル右上 × でクローズ可能（`<SuggestPanel />` に `onClose` prop を追加）
- [ ] プレフィル成立で自動クローズ（ユーザーの視線を録音ボタンに戻す）
- [ ] 自動表示（before_chat / during_chat の auto push）を削除
- [ ] ユーザー設定（`suggest_timing` / `user_settings` テーブル）を導入しない
- [ ] `suggest_requested` PostHog イベント発火
- [ ] 既存 `suggest_shown` / `suggest_clicked` は維持
- [ ] `/api/suggest` に変更なし（クライアント側の呼び出しタイミングだけが変わる）
- [ ] 3.6 節不可侵項目に変更なし
...
前提：1 アクティブユーザー / 日 ≒ 1,200 文字程度（会話 864 + 独立画面 480 + サジェスト 202 のうち、ユーザーが実際にタップ再生するもののみ計上。自動再生は提供しないため 70% 係数は使わない）。
...
**サジェスト自動再生**：**提供しない**（ユーザーが再生ボタンをタップしたときのみ ElevenLabs を呼ぶ）。上表のコスト試算はこの前提で再計算する必要がある（β期以降の実測でプラン移行を判断）。
...
4. TTS 品質（`eleven_multilingual_v2` で 5〜15 単語が破綻なく再生される。学習再生で `eleven_v3` に切り替わっても同等の自然さが保たれる）
...
8. サジェストが会話前 / 中に出現し、ユーザー設定（タイミング）が機能する。サジェストは `/api/suggest` で生成されており、`/api/chat` にロジック混入がない。
