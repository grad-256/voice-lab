...
| ゲスト制限（`lib/guestUsage.ts` の GUEST_LIMIT） | 維持。カウント対象に分身再生・サジェスト採用を追加 |
...
- 英語フレーズ（1〜3 個）
- 各フレーズに [ ▶ 分身の声で聞く ] ボタン
- [ このフレーズで話す ] ボタン（会話入力欄にプレフィル。MVP ではこれで十分）
...
- ルート名案：`/voice-library` / `/my-voice` / `/listen` から Sprint 0 で branding と擦り合わせ（Q9）。
- タブ構成：
...
- **特徴量抽出（クライアント側）**：**Meyda.js** または **Web Audio API** をブラウザ内で使う。抽出項目：
  - ピッチ中心値（F0 平均）
  - トーン明度（スペクトル重心 / MFCC 先頭数次元）
  - テンポ（発話速度 / 短時間エネルギー変動）
  - 抽出完了後、**録音 Blob は即破棄**（`URL.revokeObjectURL` + 参照削除）。
- **類似度マッチング**：クライアント側でコサイン類似度計算（Q8 で (a) クライアント完結を推奨）。上位 3 つの `{ voice_id, score, reason_text }` をステートに保持。
...
- 既存値をそのまま流用（Q2 で再検証）。
- 初期値は既存のまま、MVP リリース後 2 週間で PostHog データを見て調整。
...
**依存**：なし
**完了条件**：
- [ ] `lib/guestUsage.ts` を読解し、既存のゲスト UUID 生成・localStorage キー規約・登録時の引き継ぎフック有無を `docs/` にメモ
- [ ] 既存フックがない場合の `onAuthStateChange` ベース移行経路の実装方針を確定（3.7 節に沿う）
- [ ] ElevenLabs 月次試算（想定 DAU × 平均再生文字数 × Voice Library クレジット単価）を infrastructure が算出（Q4）
- [ ] 独立画面のルート名を branding と擦り合わせて確定（Q9：`/voice-library` / `/my-voice` / `/listen` から選定）
- [ ] プリセット voice 5〜10 個の候補洗い出し草案（marketer + branding、Q11）
- [ ] プリセット場面 5〜10 個の候補草案（marketer、Q1）
- [ ] legal-checker に 3.10 節の同意文言をレビュー依頼（Q7）
- [ ] Open Questions Q1/Q3/Q4/Q7/Q8/Q9/Q10/Q11 の暫定回答を仕様書末尾に追記
...
- [ ] 最低 8 秒未満はやり直し促し
- [ ] ブラウザ内で特徴量抽出（Meyda.js または Web Audio API）
...
- Sprint 0 が起点（既存コード読解・Open Questions 回答が全ての前提）
...
## 7. Open Questions（Masaru さんに確認すべき論点）

### Q1. プリセット場面の確定リスト（5〜10 個）

- 本書では例示 5 個（駅で道、トイレ、会計、乗り換え、空港荷物）。最終 5〜10 個を誰が決めるか。
- **推奨**：marketer エージェントに「とっさの英語が出てこない人」ペルソナ視点で 10 個の案を草案 → Masaru さん最終選定。
- **担当**：marketer + Masaru、Sprint 2 着手前に確定。Sprint 0 で草案着手。

### Q2. ゲスト制限のしきい値（既存の流用 or 変更）

- `lib/guestUsage.ts` の既存 GUEST_LIMIT をそのまま使うか、分身再生 / サジェスト採用を加算したうえでしきい値を調整するか。
- **推奨**：既存値をそのまま流用し、MVP リリース後 2 週間の PostHog データで判断。
- **担当**：pdm + Masaru。

### Q3. サジェスト生成方法（Claude Haiku ベース or ルールベース）

- MVP ではどちらを採用するか。両方用意して API 障害時のフォールバックとするか。
- **推奨**：メインは Claude Haiku（`claude-haiku-4-5-20251001`）ベース、API 障害時はプリセット 3 フレーズのルールベースでフォールバック。
- **担当**：backend + Masaru、Sprint 4 着手前。

### Q4. ElevenLabs 課金プラン確認（Starter 継続 or Creator へ移行）

- Voice Library 利用は slot 消費なし。残るはクレジット（1 文字 = 1 credit）。
- Starter（月 30,000 字 / $6）で 1〜2 人ベータは足りるが、ベータ拡大時は要見直し。
- **推奨**：infrastructure エージェントに想定 DAU × 平均再生文字数で月次試算を Sprint 0 で実施。
- **担当**：infrastructure + Masaru、Sprint 0。

### Q5. `phrase_used_in_chat` の突合仕様（確定済・4.5 節に本体）

- 2 段階キー（`phrase_id` 直接突合 / 正規化テキスト一致）で判定する方針を採用。
- 正規化ルール・保持期間・実装配置は 4.5 節を参照。
- **残論点**：縮約形展開辞書の初期語彙（MVP は最小辞書：`don't`, `it's`, `can't`, `I'm`, `you're`, `they're`, `won't`, `isn't`, `doesn't` 程度で十分か）。
- **担当**：frontend + backend、Sprint 4 着手前に語彙リスト確定。

### Q6. サジェスト API の実装方式（確定：`/api/suggest` 新設）

- `/api/chat` 拡張 vs `/api/suggest` 新設の選択肢で、**`/api/suggest` 新設で確定**（決定事項 14）。
- 理由：`/api/chat` の責務（会話返答）とサジェスト生成の責務分離、将来のプロンプト独立チューニング、障害切り分けの容易さ。
- **残論点**：レート制限・キャッシュ戦略（同じ `persona_id` + 直近履歴での重複生成を抑える）。
- **担当**：backend、Sprint 4 着手前。

### Q7. 法務同意文言（legal-checker 確認）

- 3.10 節の最小版で十分か、追加文言が必要か。
- 既存プラポリ・利用規約への追記（録音は保存しない旨）が必要か。
- **担当**：legal-checker、Sprint 1 着手前に必ずレビュー。Sprint 0 で着手。

### Q8. 特徴量抽出の実装場所（Meyda.js クライアント vs サーバー）

- **選択肢**：
  - (a) クライアント完結（Meyda.js 等で抽出 + 類似度計算もブラウザ）
  - (b) ハイブリッド（抽出はブラウザ、類似度計算は `/api/voice-match`）
  - (c) サーバー抽出（録音をサーバー送信して抽出）→ **禁止事項 3.9 に抵触するため不採用**
- **推奨**：(a)。3.9 節の禁止事項と最も整合し、レイテンシも最小。プリセット voice 数が増えた場合に (b) を検討。
- **担当**：frontend + backend、Sprint 1 着手前。

### Q9. 独立画面のルート名

- 候補：`/voice-library` / `/my-voice` / `/listen`。
- **推奨**：branding と marketer の擦り合わせで Sprint 0 に確定。ブランド世界観（「分身」「慣れる」）に整合する名称を優先。
- **担当**：branding + marketer + Masaru、Sprint 0。

### Q10. 分身の声の再マッチング頻度制限（コスト管理）

- 再マッチングは ElevenLabs TTS プレビュー（候補 3 つの短フレーズ再生）を伴うためクレジット消費あり。
- ユーザーが無制限に再マッチングすると想定外コストが発生しうる。
- **推奨**：MVP では頻度制限なし（UI 動線を控えめにするに留める）。Pro プラン移行後のコストを見て再判断。
- **担当**：pdm + infrastructure + Masaru。

### Q11. プリセット voice 5〜10 個の選定（ElevenLabs Voice Library）

- 想定構成：男性 × 3（若年/中年/年配）、女性 × 3（若年/中年/年配）、中性的 × 2。
- 各 voice に属性タグ（ピッチ / 明度 / テンポ / 性別 / 年代）を人手測定して JSON 化する必要あり。
- **選定基準の論点**：「日本語話者に親しみやすい英語発音」を優先するか、「ネイティブらしさ」を優先するか。
- **推奨**：marketer + branding が候補洗い出し → Masaru 最終選定 → 属性タグは Claude 協力で短い発話サンプルから測定。
- **担当**：marketer + branding + Masaru、Sprint 0 で草案・Sprint 1 着手前に確定。
